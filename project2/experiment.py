...
def experiment(exp_name, time_trace, bandwidth_trace, video, verbose_output=False):
        ...
        if min_time == state['playback_change_time']:
            ...
            if state['data'][state['playback_chunk'] + 1] != -1:
                ...
                if nxt_quality != state['next_chunk_quality'] or nxt_chunk != state['next_chunk']:
                    ...
                    logger.debug("{}s: Setting the new download for chunk {} at quality level {}".format(state['time'],
                                                                                                         nxt_chunk,
                                                                                                         nxt_quality))
                    set_download(state, nxt_quality, nxt_chunk, timeout)
                else:
            ...
            if state['next_chunk'] != -1:
                state['next_chunk_expected'] = (video[state['next_chunk_quality']][state['next_chunk']] - state[
                    'next_chunk_downloaded']) / state['bandwidth'] + state['time']
                logger.debug(
                    "{}s: With constant bandwidth, chunk {} download will terminate at {}".format(state['time'],
                                                                                                  state['next_chunk'],
                                                                                                  state[
                                                                                                      'next_chunk_expected']))
            ...
            if nxt_quality != state['next_chunk_quality'] or nxt_chunk != state['next_chunk']:
                logger.debug("{}s: ABR modified decision".format(state['time']))
                logger.debug(
                    "{}s: Setting the new download for chunk {} at quality level {}".format(state['time'], nxt_chunk,
                                                                                            nxt_quality))
                set_download(state, nxt_quality, nxt_chunk, timeout)
            else:
                logger.debug("{}s: ABR didn't modify decision".format(state['time']))
    ...
    switches_amplitude = sum(abs(BITRATES_REWARD[state['data'][i - 1]] - BITRATES_REWARD[state['data'][i]]) for i in
                             range(len(state['data'])))
